...
- Pin the callee ref (**prefer `@v3.0.1` or a commit SHA**). Avoid `@main` in production CI so standards updates do not surprise you.
...
jobs:
  pre-commit:
    uses: YOUR_ORG/python-project-standards/.github/workflows/reusable-pre-commit.yml@v3.0.1
    with:
      python-version: "3.14"
      install-command: |
