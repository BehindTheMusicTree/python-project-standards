...
- Pin the callee ref (**prefer `@v4.1.1` or a commit SHA**). Avoid `@main` in production CI so standards updates do not surprise you.
...
jobs:
  pre-commit:
    uses: YOUR_ORG/python-project-standards/.github/workflows/reusable-pre-commit.yml@v4.1.1
    with:
      python-version: "3.14"
      install-command: |
