...
- **python-project-standards** keeps **`.claude/rules/*.md`** (Claude Code format: `paths:` frontmatter instead of `globs`) in lockstep with **`templates/cursor-rules/*.mdc`** (same rules for maintainers; templates use absolute GitHub links where useful for copy-paste consumers, while **`.claude/rules/strenum-string-enums.md`** points at **`docs/`** with repo-relative paths). String enums policy is also in **[string-enums.md](string-enums.md)**.
