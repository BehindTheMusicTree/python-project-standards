...
```yaml
uses: BehindTheMusicTree/python-project-standards/.github/workflows/reusable-pre-commit.yml@v4.1.0
```
...
Pin **`reusable-pre-commit.yml`** to a current release tag (e.g. **`@v4.1.0`**) when adopting **v4+**; it is unrelated to the removed matrix but should match **`STANDARDS_VERSION`** for org consistency.
