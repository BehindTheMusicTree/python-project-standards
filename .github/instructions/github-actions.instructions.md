...
- **`lint.yml`** is the **Tier A** pattern: a single job that **`uses:`** the org reusable workflow with a **pinned ref** (example in tree: `@v3.0.1`). When updating the example pin, consider whether **`STANDARDS_VERSION`**, **`CHANGELOG.md`**, and docs examples need the same bump for consistency.
