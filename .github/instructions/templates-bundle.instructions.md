...
- Keep **`requires-python`**, **`[tool.ruff]` `target-version`**, and **`[tool.mypy]` `python_version`** coherent (baseline targets **3.12** in the current template).
...
- **`[tool.ruff.lint]` `select`** should keep **`UP`** so **`UP042`** / `StrEnum` guidance in `docs/string-enums.md` matches enforcement.
