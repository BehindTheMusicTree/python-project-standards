...
- Keep the **early exit** path for **python-project-standards** itself (`templates/pyproject/pyproject.toml` exists **and** root **`baselines/ruff.toml`** is absent): consumers must not break, and this repo’s own CI or hooks must not falsely fail.
